...
def convert_videos_to_IAD(file_reader, c3d_depth, records=None):
	...
	value_array_size = [64, 128, 256, 512, 1024]

	max_vals, min_vals = None, None
	if(records):
		max_vals, min_vals = RawArray('d', value_array_size[c3d_depth]), RawArray('d', value_array_size[c3d_depth])
		for i in range(value_array_size[c3d_depth]):
...
if __name__ == '__main__':
	...
	if(len(sys.argv) == 2):
		thresholding_options = ['local_norm', 'global_norm']

		thresholding_approach = sys.argv[1]
		assert thresholding_approach in thresholding_options, "thresholding approach must be in "+str(thresholding_options)
		...
		if(thresholding_approach == 'local_norm'):

			thresholding_approach = "norm"

			for c3d_depth in range(5):#5):
		...
		elif(thresholding_approach == 'global_norm'):
		...
		print("Usage: python iad_gen_global_norm.py <norm_method>")
		print("\tlocal_norm")
		print("\tglobal_norm")
